    ...
    def add_slime_arguments(parser):
        ...
        def add_cluster_arguments(parser):
            ...
            parser.add_argument(
                ...
                action="store_true",
                default=False,
                help="Enable separate GRPO training for schema linking. "
                    "When enabled, the model will be trained on both full SQL trajectories "
                    "and truncated schema linking trajectories."
            )
            parser.add_argument(
                "--use_segmented_propagation",
                action="store_true",
                default=False,
                help="Enable segmented propagation for token-level REINFORCE++ rewards. "
                    "When disabled (default): Uses global propagation - SQL rewards propagate to Schema tokens "
                    "through discount mechanism (gamma), allowing SQL success to influence Schema learning. "
                    "When enabled: Uses segmented propagation - Schema and SQL compute discounted returns "
                    "independently at schema_end_position, preventing cross-influence. "
                    "Only effective when enable_schema=True and advantage_estimator='reinforce_plus_plus'. "
                    "Recommended: Start with global propagation (default) for most cases."
            )
            parser.add_argument(
                "--use_weighted_schema_advantage",
                action="store_true",
                default=False,
                help="Enable weighted schema+answer advantages for segmented GRPO/GSPO. "
                    "When enabled: compute schema advantage from schema rewards and answer advantage from final reward; "
                    "apply answer advantage to all tokens and add weighted schema advantage to schema tokens. "
                    "Only effective when advantage_estimator is grpo/gspo and schema_end_positions are available."
            )
            parser.add_argument(
                "--schema_advantage_weight",
                type=float,
                default=1.0,
                help="Weight for schema advantage when using --use_weighted_schema_advantage."
            )
            parser.add_argument(
                "--use_weighted_schema_policy_loss",
                action="store_true",
                default=False,
                help="Enable separate schema/answer policy losses for segmented GRPO/GSPO. "
                    "When enabled: compute schema advantage from schema rewards and answer advantage from final reward; "
                    "compute a schema-only policy loss (masking SQL tokens) and a full-response policy loss, "
                    "then combine them with weights. Only effective when advantage_estimator is grpo/gspo and "
                    "schema_end_positions are available."
            )
            parser.add_argument(
                "--schema_policy_loss_weight",
                type=float,
                default=1.0,
                help="Weight for schema policy loss when using --use_weighted_schema_policy_loss."
            )
            parser.add_argument(
                "--answer_policy_loss_weight",
                type=float,
                default=1.0,
                help="Weight for answer policy loss when using --use_weighted_schema_policy_loss."
            )
            # 添加参数解析
            parser.add_argument(
                "--schema_scoring_mode",
                ...
                help="Schema Linking 评分模式: f1 (默认), precision (准确率), recall (召回率)"
            )
            parser.add_argument(
                "--rollout-num-gpus",
                type=int,
