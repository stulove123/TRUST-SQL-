...
def process_rollout_data(args, rollout_data_ref, dp_rank, dp_size):
    ...
    if "schema_data" in data:
        ...
        if "tokens" in schema_data and len(schema_data["tokens"]) > 0:
            num_schema_samples = len(schema_data["tokens"])
            
            # 对 schema_data 使用简单轮询分区
            def get_schema_partition(val):
                return val[dp_rank::dp_size]
            
            # 【关键修改】展平 schema_data，使用 schema_ 前缀
            for key in schema_data:
                if isinstance(schema_data[key], list):
                    # 分区后直接添加到 rollout_data，使用 schema_ 前缀
                    rollout_data[f"schema_{key}"] = get_schema_partition(schema_data[key])
                else:
                    # 非列表类型直接复制
                    rollout_data[f"schema_{key}"] = schema_data[key]
            
            # 调试日志
            if rank == 0 and getattr(args, 'enable_schema_grpo', False):
                partitioned_count = len(rollout_data["schema_tokens"])
                print(f"[DEBUG process_rollout_data] Schema data flattened and partitioned: "
                      f"Total={num_schema_samples}, Rank {dp_rank} got {partitioned_count} samples")
                print(f"[DEBUG] Schema keys added to rollout_data: {[k for k in rollout_data.keys() if k.startswith('schema_')]}")
