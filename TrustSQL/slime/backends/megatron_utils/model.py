...
def train_one_step(
    ...
    rollout_metadata: dict = None,  # ✅ 添加这个参数
    ...
    Args:
        ...
        rollout_metadata (dict): Metadata for expanded trajectories.  # ✅ 新增
        ...
        if rollout_metadata:
            for key, value in rollout_metadata.items():
                if value is not None:
                    batch[key] = value
            if step_id == 0:  # 只在第一个 step 打印一次
                print(f"[FORWARD STEP] Added metadata to batch: {list(rollout_metadata.keys())}", flush=True)
...
def train(
    ...
    Args:
        ...
        rollout_data (dict): Complete rollout data including metadata.  # ✅ 新增
    """
    args = get_args()
    ...
    if rollout_data is not None:
        is_expanded = rollout_data.get("is_expanded", False)
        if is_expanded:
            rollout_metadata = {
                "is_expanded": True,
                "original_response_lens": rollout_data.get("original_response_lens"),
                "original_schema_ends": rollout_data.get("original_schema_ends"),
                "use_schema_sql_split": rollout_data.get("use_schema_sql_split"),
                "expansion_map": rollout_data.get("expansion_map"),
            }
            print(f"[TRAIN] Extracted metadata for expanded data", flush=True)
            print(f"[TRAIN] Metadata keys: {list(rollout_metadata.keys())}", flush=True)
    ...
    for step_id in range(num_steps_per_rollout):
        ...
        # ✅ Run training step with metadata
        loss_dict, grad_norm = train_one_step(
            ...
            rollout_metadata=rollout_metadata,  # ✅ 传递元数据
        )
